class RecentOpponents(m: Map[String, Double], s: Map[(String, String), Double]) {
  def updated(u1: String, u2: String): RecentOpponents =
    if (u1 > u2) updated(u2, u1)
      ...
      new RecentOpponents(m.updated(u1, s1 * 0.5).updated(u2, s2 * 0.5), s.updated((u1, u2), x + s1.max(s2)))
    }
...
object RecentOpponents {
  def empty: RecentOpponents =
    new RecentOpponents(Map.empty[String, Double], Map.empty[(String, String), Double])
}
